...
// Insert the NTML Code for the first puzzle table
...
function swapPuzzle(e) {
   ...
   // switch statment to determine which puzzle to actually draw based on which button was clicked
   switch(puzzleID) {
      case "puzzle1":
         document.getElementById("puzzle").innerHTML = drawPuzzle(puzzle1Hint, puzzle1Rating, puzzle1);
         break;
...
function setupPuzzle() {
   // Refernce all of the data cells in the puzle table
   puzzleCells = document.querySelectorAll("table#hanjieGrid td");

   // loop through the puzzleCells array setting the color gold
   for(let i = 0;  i < puzzleCells.length; i++) {
      puzzleCells[i].style.backgroundColor = "rgb(233, 207, 29)";
      // set up each cell with mousedown event handler
   ...
   // create an event listener to higlight incorrect cells
   document.getElementById("peek").addEventListener("click", function(){
      //Dispay incorrect white cells in pink
      for(let i = o; i < filled.length; i++) {
      ...
      for(let i = o; i < empty.length; i++) {
         if(empty[i].style.backgroundColor === "rgb(101, 101, 101") {
            empty[i].style.backgroundColor = "rgb(255, 101, 101)";
         }// end of if statement
      } // end of for loop

      // Remove the hints aftera certain amount of time
      setTimeout(function() {
         for(let i = 0; i < puzzleCells.length; i++) {
            if(puzzleCells[1].style.backgroundColor === "rgb(255, 211, 211") {
               puzzleCells.style.backgroundColor = "rgb(255, 255, 255)";
            }

            if(puzzleCells[1].style.backgroundColor === "rgb(255, 101, 101") {
               puzzleCells.style.backgroundColor = "rgb(101, 101, 101)";
            }
   ...
   document.getElementById("hanjieGrid").addEventListener("mouseup",function(){
         ...
         for(let i = 0; i < puzzleCells.length; i++) {
            if((puzzleCells[i].className === "filled" && puzzleCells[i].style.backgroundColor !== "rgb(101, 101, 101)") || (puzzleCells.className === "empty" && puzzleCells[i].style.backgroundColor === "rgb(101, 101, 101)")) {
               solved = false;
               break;
...
function setBackground(e) {
   ...
   // loop through puzzleCells array and create and event listener for all of them
   for(let i = 0; i < puzzleCells.length; i++){
...
// Definition of the extendBackround() function which keeps the background color going
